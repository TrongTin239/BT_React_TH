...
export const bookingTicketReducer = (state = stateDefault, action) => {
  switch (action.type) {
    case "BOOKING_CHAIR": {
      ...
      // console.log(listBookingUpdate)
      let index = listBookingUpdate.findIndex(
      ...
      if (index !== -1) {
        ...
        listBookingUpdate.push(ghe);
      }
      state.listBooking = listBookingUpdate;
      return { ...state };
    }
    case "CANCLE_CHAIR": {
      let {soGhe} = action;
      ...
      let index = listBookingUpdate.findIndex((ghe) => ghe.soGhe === soGhe) ;
      console.log(index);
      if (index !== -1) {
        listBookingUpdate.splice(index, 1);
      }
